...
此 GC 在程序开始后的 17.806 秒发生。年轻代中的对象现在占据了 14,463 KB（14 MB，在幸存者空间中）；在 GC 之前，它们占据了 227,983 KB（227 MB）。^(1) 此时年轻代的总大小为 264 MB。
...
^(1) 实际上，227,893 KB 只有 222 MB。为了方便讨论，在本章中，我将以 1,000 为单位截断 KB；假装我是一个磁盘制造商。

^(2) 类似的工作也可以使 CMS 全 GC 使用并行线程运行，但 G1 GC 的工作被优先考虑。

^(3) 这是线程本地变量可以防止锁竞争的一种变体方法（参见第九章）。
