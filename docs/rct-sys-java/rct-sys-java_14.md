...
响应式消息传递提供了多种生成消息的方式，但让我们从最简单且可能更符合熟悉命令式编程的开发者的方式开始：发射器。`Emitter`是一个附加到通道并将消息发射到该通道的对象。¹ 示例 10-1 说明了使用`MutinyEmitter`发送消息的方法。
...
在这个片段中，我们从`from`通道读取字符串。对于每个接收到的字符串，我们创建一个`Person`实例，然后将其发送到`to`通道。这种方法是同步的，接受一个个体负载并返回一个个体负载。这并不是唯一支持的签名方式。响应式消息支持超过 30 种签名，允许异步处理（例如返回`Uni`），甚至流处理（其中你接收并返回`Multi`），²参见 Example 10-12。
...
² [您可以在 GitHub 上找到支持的签名列表](https://oreil.ly/hewBO)。
