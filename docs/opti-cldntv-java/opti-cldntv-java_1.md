...
讨论假设读者熟悉基本的正态分布测量统计处理（均值、模式、标准差等）；若读者不熟悉，应参考基础教材，比如[*生物统计手册*](http://biostathandbook.com/)³。
...
在最极端的例子中，如果从业者足够努力地寻找，就可以发现[完全不相关的测量结果](http://tylervigen.com/spurious-correlations)之间存在关联。例如，在图 2-4 中，我们可以看到美国的鸡肉消费与原油总进口存在很好的相关性。⁴
